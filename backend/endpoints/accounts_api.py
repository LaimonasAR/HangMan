...
@router.put("/password/{account_id}", response_model=AccountResponse)
def update_account(
    password: PasswordUpdate, account_id: int, db: Session = Depends(get_db)
):
    try:
        db_account = crud.account_crud.update_password(
            db, account_id=account_id, password=password
        ) 
        return db_account
    except NoResultFound:
        raise HTTPException(status_code=404, detail=f"account {account_id} not found")
